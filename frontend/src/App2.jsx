...
const TIMES_OF_DAY = [
  { id: "day", label: "Daytime", icon: "☀️", desc: "Natural daylight" },
  { id: "night", label: "Night Time", icon: "💡", desc: "Interior light" },
];
...
function TimeOfDaySelector({ timeOfDay, setTimeOfDay }) {
  return (
    <div style={{ display: "flex", gap: 10, justifyContent: "center" }}>
      {TIMES_OF_DAY.map(t => (
        <button
          ...
          style={{
            ...
            cursor: "pointer",
            transition: "all 0.2s",
            border: timeOfDay === t.id ? "2px solid #7a6344" : "2px solid #d8d0c4",
            background: timeOfDay === t.id ? "rgba(122,99,68,0.08)" : "#fff",
            color: timeOfDay === t.id ? "#4a3820" : "#8a7e6e",
            fontWeight: timeOfDay === t.id ? 600 : 400,
            fontSize: 13,
            display: "flex",
            alignItems: "center",
            gap: 6,
...
// Side-by-side quick preview (keep window & curtain separate)
function QuickPreview({ imageUrl, style, mode, timeOfDay }) {
  if (!style) return null;
  const modeLabel = MODES.find(m => m.id === mode)?.label || "";
  const timeLabel = TIMES_OF_DAY.find(t => t.id === timeOfDay)?.label || "";
  ...
  return (
    ...
    >
      ...
      {/* Curtain style card (independent from photo) */}
      <div
        style={{
          flex: 1,
          minWidth: 260,
        ...
        >
          ...
          <div
            style={{
              ...
              height: 12,
              background:
                "linear-gradient(180deg, #bba882, #9a896a, #bba882)",
            }}
          />
          {/* Folds */}
          {[18, 42, 66, 90, 114].map((x, i) => (
            <div
              ...
              style={{
                ...
                background:
                  "linear-gradient(180deg, transparent, rgba(0,0,0,0.06) 30%, rgba(255,255,255,0.06) 50%, rgba(0,0,0,0.06) 70%, transparent)",
              }}
            />
          ))}
...
export default function CurtainVisionApp() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (!resp.ok) {
        const message =
          data.detail || data.error || `Server error (${resp.status})`;
  ...
  return (
    <div style={{
      ...
      <main key={fadeKey} style={{
        maxWidth: 1120, margin: "0 auto", padding: "32px 16px",
        ...
        {step === 2 && (
          <div>
            ...
            <div style={{ marginBottom: 12 }}><ModeSelector mode={mode} setMode={setMode} /></div>
            <div style={{ marginBottom: 20 }}><TimeOfDaySelector timeOfDay={timeOfDay} setTimeOfDay={setTimeOfDay} /></div>
            <QuickPreview imageUrl={imageUrl} style={style} mode={mode} timeOfDay={timeOfDay} />
          ...
          <div style={{ textAlign: "center" }}>
            ...
            <div style={{ maxWidth: 600, margin: "0 auto" }}>
              ...
              <div style={{ display: "flex", gap: 16, marginTop: 20, justifyContent: "center", flexWrap: "wrap" }}>
                <div style={{ flex: 1, minWidth: 240, maxWidth: 280 }}>
