...
async def generate_curtain(
    ...
    try:
        ...
        # 窗户照片转 base64
        window_data_uri = image_to_data_uri(resized, "image/png")

        # 参考图转 base64
        ref_data_uris = []
        for ref_filename in CURTAIN_STYLES[style]["refs"]:
            ref_path = RESOURCES_DIR / ref_filename
            if ref_path.exists():
                ref_data_uris.append(file_to_data_uri(ref_path))
            else:
                print("[Warning] ref file not found: " + str(ref_path))

        # 组合：窗户照片 + 参考图
        all_images = [window_data_uri] + ref_data_uris
        ...
        print("[Generating] style=" + style + " file=" + file_id + " refs=" + str(len(ref_data_uris)))
